...
The LCM project provides pubsub clients and code generators for many languages. For us the real power of LCM is its message definition format — classes that encode themselves to a compact binary representation. This means LCM messages can be sent over any transport (WebSocket, SSH, shared memory, etc.), not just UDP multicast.

Our types are ported from ROS (in order to make conversion of our messages to ROS messages easy) and autogenerated from the [dimos-lcm](https://github.com/dimensionalOS/dimos-lcm/) repository.
